  ...
  for(j in 1:length(taxa_names)) {
    ...
    plot_list[[p_index]] <- ggplot() +
      geom_rect(data = moc_conc[[i]][moc_conc[[i]]$taxa == taxa,],
                ...
                alpha = .5)+
      geom_rect(data = uvp_list[[i]][uvp_list[[i]]$taxa == taxa,],
                aes(xmin = min_d, xmax = max_d,
                    ymin = 0, ymax = conc_m3,
                    fill = 'UVP', color = 'UVP'),
                alpha = .5)+
      scale_color_manual(values = gg_cbb_col(2), breaks = c('Mocness','UVP'))+
...
names(plot_list) <- paste(rep(profile_names, each = length(taxa_names)), 
                          taxa_names,sep = '_')
saveRDS(plot_list, './Output/main_fig03_profile-density-comparison.R')
