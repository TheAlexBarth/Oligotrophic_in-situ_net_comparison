####
# Analysis - Regression for matching depth bins of biomass concentration
####
...
uvp_list <- readRDS('./Data/uvp_all-binned-biomass_matched.rds')
...
for(i in 1:length(taxa_names)) {
  ...
  pool_plot[[i]] <- ggplot(pooled_all[pooled_all$taxa == taxa,])+
    ...
    labs(x= bquote(MOCNESS~'[mg'~m^-3*']'), y = bquote(UVP~'[mg'~m^-3*']'),
         subtitle = taxa,
         col = '')+
...
saveRDS(list(
  pooled = pool_plot,
  avged = avg_plot
), './Output/main_fig_07_biomass-regression-plots.rds')
saveRDS(list(
  pooled = pool_regressions,
  avged = avg_regressions
), './Output/data_07_biomass-regressions-models.rds')