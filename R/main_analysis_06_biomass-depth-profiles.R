...
uvp_list <- readRDS('./Data/uvp_all-binned-biomass_indp.rds')

taxa_names <- c('Chaetognatha','Copepoda', 'Shrimp-like', 'Ostra/Clado')
...
for(i in 1:length(profile_names)) {
  for(j in 1:length(taxa_names)) {
    ...
    plot_list[[p_index]] <- ggplot() +
      ...
      scale_x_reverse()+
      coord_flip()+
      labs(x = 'Depth [m]', y = bquote(.(taxa) ~ '[#indv.'~m^-2*']'),
...
names(plot_list) <- paste(rep(profile_names, each = length(taxa_names)),
                          taxa_names, sep = "_")
saveRDS(plot_list, './Output/main_fig_06_profile-biomass-comparison.rds')
